...
| GIFS_PUBLIC_ORIGIN                | unset         | The origin share links are built from, for example https://gifs.example.com. Unset falls back to the requesting Host header, so set it when the app sits behind a proxy or a shared cache.                              |
...
| GIFS_MAX_FILE_SIZE_MB             | 15            | The largest file in megabytes that an upload or an import may produce. The importer refuses a download whose advertised or actual size passes it, so an oversized file is rejected rather than fetched in full.         |
| GIFS_MAX_MEGAPIXELS               | 50            | The decoded pixel budget, width times height summed across every frame. Guards against a small file that expands into an enormous canvas, which a byte limit alone cannot catch.                                        |
| GIFS_MEDIA_TIMEOUT_SECONDS        | 60            | How long any one downloader or converter may run before it is killed. Applies to gallery-dl, ffmpeg, and every ImageMagick call.                                                                                        |
...
| DISCORD_RATE_LIMIT_MAX            | 10            | How many gifs one Discord user may import per window. Anything over the allowance is skipped and the reply says when the window resets.                                                                                 |
| DISCORD_RATE_LIMIT_WINDOW_MINUTES | 10            | The length of that window in minutes.                                                                                                                                                                                   |
